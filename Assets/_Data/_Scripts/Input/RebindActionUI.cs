...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            foreach (InputAction otherAction in action.actionMap.actions)
            {
                ...
                for ( int i = 0; i < otherAction.bindings.Count; ++i)
                {
                    ...
                    if (otherBinding.overridePath == newBinding.path)
                    {
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        {
                            Debug.Log($"No duplicate binding found for {action.bindings[bindingIndex].effectivePath}");
                        }
                        UpdateBindingDisplay();
                        CleanUp();
        ...
        {
            InputBinding newBinding = action.bindings[bindingIndex];
            foreach (var otherBinding in action.actionMap.bindings)
            {
                if (otherBinding.action == newBinding.action)
                    continue;
                if (otherBinding.effectivePath == newBinding.effectivePath)
                {
                    Debug.Log($"Duplicate binding found for {action.bindings[bindingIndex].effectivePath}");
                    return true;
                }
            }
            ...
            if(allCompositeParts)
            {
                for (var i = 1; i < bindingIndex; ++i)
                {
                    ...
                    {
                        Debug.Log($"Duplicate binding found for {action.bindings[i].effectivePath}");
                        return true;
                    }
                }
            }
        ...
        [Tooltip("Optional bool field which allows you to OVERRIDE the action label with your own custom label.")]
        public bool m_OverrideActionLabel;

        [Tooltip("Text will override the action label with your own custom label.")]
        [SerializeField]
        private string m_ActionLabelString;

        private bool m_OnlyShowEnabledBindings;
        ...
        private void UpdateActionLabel()
        {
            ...
            {
                var action = m_Action?.action;
                if (m_OverrideActionLabel && !string.IsNullOrEmpty(m_ActionLabelString))
                {
                    m_ActionLabel.text = Localization.Get($"menu.options.controls.{m_ActionLabelString.ToLower().Replace(" ", "_")}");
                }
                else
                {
